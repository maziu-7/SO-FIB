    ...
    while (not Q.empty()) {
        ...
        for (int i = 0; i < C.size(); ++i) {
            ...
            if (in_range(G, sig) and G[sig.first][sig.second] != 'X') {
                //si la pos siguiente es una 't', actualizamos dmax
                if (G[sig.first][sig.second] == 't') {
                    if (dmax < d + 1) dmax = d + 1;
                }
                //añadimos la pos que estamos mirando y la marcamos con una 'X' (ya no nos interesa)
                Q.push({sig, d+1});
...
int main() {
    ...
    cin >> r >> c;
    int p = DFS(G, r, c);
    if (p == 0) cout << "no treasure can be reached" << endl;
    else cout << "maximum distance: " << p << endl;
}